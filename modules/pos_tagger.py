...
class POSTagger:
    def __init__(self):
        self.pos_tag_mapping = {
            ...
            'DT': '관사',  # Determiner (the, a, an)
            'EX': '대명사',  # Existential there (there)
            ...
            'PDT': '전치한정사',  # Predeterminer (all, both, half)
            ...
            'VBZ': '현재형 3인칭 단수 동사',  # Verb 3rd person singular present (runs, eats)
            'WDT': '관계대명사',  # Wh-determiner (which, that)
            'WP': '관계대명사',  # Wh-pronoun (who, what)
            'WP$': '소유형 관계대명사',  # Possessive wh-pronoun (whose)
            'WRB': '관계부사',  # Wh-adverb (where, when)
        }
